...
console.log("LanceDB exports:", lancedb);
...
const { schema, Utf8, vector, Float32, Int32, Any } = lancedb;
...
const LanceDb = {
  ...
  updateOrCreateCollection: async function (client, data = [], namespace) {
    ...
    if (hasNamespace) {
      const collection = await client.openTable(namespace);
      console.log("[LanceDB DEBUG] Payload to add:", JSON.stringify(data, null, 2));
      await collection.add(data);
      // Add INVERTED index for full text search
      if (collection && typeof collection.createIndex === 'function') {
        try {
          await collection.createIndex('text', 'INVERTED');
          console.log('Created INVERTED index on text for full text search.');
        } catch (err) {
          console.error('Failed to create INVERTED index:', err.message);
        }
      }
      // Debug: print all keys in every record before insertion
      console.log("[LanceDB DEBUG] All keys in batch:", data.map(r => Object.keys(r)));
      return true;
    }

    // Only allow id, vector, text, metadata as top-level fields
    for (let i = 0; i < data.length; i++) {
      const allowed = ['id', 'vector', 'text', 'metadata'];
      ...
      if (!data[i].metadata || typeof data[i].metadata !== 'object') {
        data[i].metadata = {};
      }
      // Force stringify metadata.row_data for every record
      if (data[i].metadata && data[i].metadata.row_data) {
        data[i].metadata.row_data = JSON.stringify(data[i].metadata.row_data);
        // Debug log to confirm stringification
        console.log('[LanceDB DEBUG] After stringify, metadata.row_data:', data[i].metadata.row_data, 'Type:', typeof data[i].metadata.row_data);
      }
    }
    console.log("[LanceDB DEBUG] First record before createTable:", JSON.stringify(data[0], null, 2));
    await client.createTable(namespace, data);
    // Add INVERTED index for full text search
    const table = await client.openTable(namespace);
    if (table && typeof table.createIndex === 'function') {
      try {
        console.log('Attempting to create INVERTED index on text. Table fields:', Object.keys(data[0] || {}));
        await table.createIndex('text', 'INVERTED');
        console.log('Created INVERTED index on text for full text search.');
      } catch (err) {
        console.error('Failed to create INVERTED index:', err.message);
      }
  ...
  addDocumentToNamespace: async function (
    ...
    try {
        ...
        if (cacheResult.exists) {
          ...
          for (const chunk of chunks) {
            chunk.forEach((chunk) => {
              ...
              // All custom fields go inside metadata
              submissions.push({
                id: id,
                vector: chunk.values,
                text: chunk.text || meta.text, // fallback if present
                metadata: { ...meta },
              });
            });
          }
      ...
      const textChunks = await textSplitter.splitText(pageContent, metadata);
