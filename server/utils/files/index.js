...
async function fileData(filePath = null) {
  ...
  console.log("[fileData] Reading file data from:", fullFilePath); // Debug log
  if (!fs.existsSync(fullFilePath) || !isWithin(documentsPath, fullFilePath))
    return null;

  const data = fs.readFileSync(fullFilePath, "utf8");
  console.log("[fileData] File data loaded:", data); // Debug log
  const parsed = JSON.parse(data);

  // PATCH: Remove numeric keys from structuredData.headers and structuredData.data[]
  if (parsed?.metadata?.structuredData) {
    const structuredData = parsed.metadata.structuredData;
    if (structuredData?.headers) {
      structuredData.headers = structuredData.headers.filter(h => isNaN(Number(h)));
    }
    if (structuredData?.data) {
      structuredData.data = structuredData.data.map(row => {
        const cleaned = {};
        for (const key in row) {
          if (isNaN(Number(key))) cleaned[key] = row[key];
        }
        return cleaned;
      });
    }
  ...
  // PATCH: If this is a CSV upload, re-chunk using importCSVDocument
  if (parsed.docSource === 'csv-upload') {
    const { importCSVDocument } = require('./csvImportService');
    // Use the file path and workspaceId from the parsed doc
    // The file path for importCSVDocument should be the original CSV, not the JSON
    // Try to extract the original CSV path from parsed.url
    let csvPath = parsed.url && parsed.url.startsWith('file://') ? parsed.url.replace('file://', '') : null;
    if (csvPath && fs.existsSync(csvPath)) {
      return importCSVDocument(csvPath, parsed.workspaceId);
    }
    // Fallback: return the parsed doc if CSV not found
    return parsed;
  }
  return parsed;
}
