...
class TextSplitter {
  ...
  async splitText(documentText, metadata = {}) {
    // CSV-specific chunking: each row is a chunk
    if (metadata?.docSource === 'csv-upload') {
      // Split by double newlines (as used in csvImportService.js)
      return documentText.split(/\n\n+/).filter(Boolean);
    }
    // Default: use the recursive splitter
