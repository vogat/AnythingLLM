...
async function streamChatWithWorkspace(
  ...
  // Process sources to include structured data for CSV files
  sources = [...sources, ...vectorSearchResults.sources].map(source => {
    if (source.metadata?.structuredData) {
      return {
        ...source,
        text: source.text || source.pageContent,
        structuredData: source.metadata.structuredData
      };
    }
    return source;
  });
  ...
  const isDataQuestion = message.toLowerCase().includes('data') || 
                        message.toLowerCase().includes('analysis') ||
                        message.toLowerCase().includes('statistics') ||
                        message.toLowerCase().includes('average') ||
                        message.toLowerCase().includes('sum') ||
                        message.toLowerCase().includes('count');

  // If this is a data question, try to query any CSV files
  if (isDataQuestion) {
    const csvSources = sources.filter(source => 
      source.metadata?.structuredData && 
      source.metadata?.structuredData.type === 'csv'
    );
    ...
    for (const source of csvSources) {
      try {
        ...
        if (queryResult.success && queryResult.result) {
          ...
          contextTexts.push(formattedResult);
          sources.push({
            id: `${source.id}-query-result`,
