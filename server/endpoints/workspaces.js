...
const { importCSVDocument } = require("../utils/files/csvImportService");
...
//       const workspaceId = request.params.slug;

//       // CSV bypass logic
//       if (originalname.endsWith(".csv")) {
//         const metadata = importCSVDocument(uploadedFilePath, workspaceId);
//         return response.status(200).json({
//           success: true,
//           message: "CSV document imported.",
//           document: metadata,
//         });
//       }

//       // Default: Collector logic
  ...
  async function (request, response) {
    try {
      ...
      if (originalname.endsWith(".csv")) {
        let metadata;
        try {
          metadata = importCSVDocument(uploadedFilePath, workspaceId);
        } catch (err) {
          console.error("CSV import failed:", err);
          return response.status(500).json({ success: false, error: "CSV import failed: " + err.message });
        }

        if (!metadata || !metadata.location) {
          console.error("CSV import did not return a valid document location.", metadata);
          return response.status(500).json({ success: false, error: "CSV import did not return a valid document location." });
        }

        // PATCH: Fetch the full workspace object so we have the slug for vectorization
        const currWorkspace = await Workspace.get({ slug: workspaceId });
        if (!currWorkspace) {
          return response.status(400).json({ success: false, error: "Workspace not found." });
        }

        let embedResult;
        try {
          embedResult = await Document.addDocuments(
            currWorkspace,
            [metadata.location],
            response.locals?.user?.id
          );
        } catch (err) {
          console.error("Embedding CSV failed:", err);
          return response.status(500).json({ success: false, error: "Embedding CSV failed: " + err.message });
        }

        const { failedToEmbed = [], errors = [] } = embedResult || {};
        if (failedToEmbed.length > 0) {
          console.error("Embedding failed for:", failedToEmbed, errors);
          return response.status(500).json({ success: false, error: errors?.[0] || "Embedding failed." });
        }

        return response.status(200).json({
          success: true,
          message: "CSV document imported and embedded.",
          document: metadata,
        });
      }
      ...
      // Now return success + document reference
      return response.status(200).json({
        success: true,
        error: null,
        document: {
          id: originalname,
          location: `custom-documents/${originalname.replace(/\.[^/.]+$/, ".json")}`,
        },
      });
    } catch (e) {
      console.error(e.message, e);
  ...
  app.post(
    ...
    async function (request, response) {
      try {
        ...
        const workspaceId = currWorkspace.id;
        ...
        const uploadedFilePath = path.join(__dirname, "../../collector/hotdir", originalname);
        ...
        if (isCSV) {
          ...
          const result = await Collector.processDocument(originalname);
          success = result.success;
          reason = result.reason;
          if (!success || result.documents?.length === 0) {
            return response.status(500).json({ success: false, error: reason, document: null });
          }
