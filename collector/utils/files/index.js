...
function writeToServerDocuments(
  ...
  // Clean invalid rows before saving to disk
  if (data?.metadata?.structuredData?.data && data?.metadata?.structuredData?.headers) {
    ...
    structuredData.data = structuredData.data.map(row => {
      ...
      const cleaned = {};
      for (let i = 0; i < structuredData.headers.length; i++) {
        cleaned[structuredData.headers[i]] = row[i];
      }
      return cleaned;
    });
  }
...
async function test() {
  const db = await lancedb.connect(path.resolve(__dirname, 'test-lancedb'));
  const table = await db.createTable('test', [
    { pageContent: 'test', docId: '1' }
  ]);
  await table.add([{ pageContent: 'another test', docId: '2' }]);
  const results = await table.search('test').limit(1).execute();
  console.log(results);
}
