...
export function DnDFileUploaderProvider({ workspace, children }) {
   ...
   * for a chat.
   * @returns {{name:string,mime:string,contentString:string,document?:object}[]}
   */
      ...
      files
        ?.filter(
          (file) =>
            ...
            (file.type === "upload" && file.status === "success" && file.document)
        )
        ?.map((attachment) => ({
          ...
          document: attachment.document || undefined,
  ...
  /**
   * Returns true if any files are still uploading/embedding
   */
  function hasPendingUploads() {
    return files.some((file) => file.type === "upload" && file.status === "in_progress");
  }
  ...
  return (
    <DndUploaderContext.Provider
      value={{ files, ready, dragging, setDragging, onDrop, parseAttachments, hasPendingUploads }}
    >
      {children}
    </DndUploaderContext.Provider>
  );
