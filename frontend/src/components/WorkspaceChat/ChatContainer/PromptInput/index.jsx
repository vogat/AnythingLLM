...
export default function PromptInput({
  ...
  const { hasPendingUploads } = useContext(DndUploaderContext);
  ...
  function captureEnterOrUndo(event) {
    ...
    if (event.keyCode === 13 && !event.shiftKey) {
      ...
      if (isStreaming || hasPendingUploads) return; // Prevent submission if streaming or disabled
      return submit(event);
    }
              ...
              {isStreaming ? (
                ...
                <>
                  <button
                    ref={formRef}
                    type="submit"
                    disabled={attachments.length > 0 && hasPendingUploads}
